from rest_framework.test import APITestCase
from rest_framework import status
from unittest import mock
from django.conf import settings
from attrdict import AttrDict


class DotDict(dict):
    """dot.notation access to dictionary attributes"""
    __getattr__ = dict.__dict__
    __setattr__ = dict.__setitem__
    __delattr__ = dict.__delitem__


class FolderItem:
    __dict__ = {'completed_date_time': None,
                'created_date_time': '2023-02-02T09:56:58.7600000Z',
                'envelope_id': 'bef86597-7e1d-4a77-a778-e8f2fe004cf3',
                'envelope_uri': '/envelopes/bef86597-7e1d-4a77-a778-e8f2fe004cf3',
                'expire_date_time': '2023-06-02T14:02:05.4753500Z',
                'folder_id': None,
                'folder_uri': None,
                'is21_cfr_part11': 'false',
                'last_modified_date_time': '2023-02-02T09:56:59.0000000Z',
                'owner_name': 'Muhammad Shoaib',
                'recipients': None,
                'recipients_uri': '/envelopes/bef86597-7e1d-4a77-a778-e8f2fe004cf3/recipients',
                'sender_company': None,
                'sender_email': 'muhammad.shoaib@crowdbotics.com',
                'sender_name': 'Muhammad Shoaib',
                'sender_user_id': 'c941de4f-d46f-4f36-b0b5-0ca24c2003fa',
                'sent_date_time': '2023-02-02T09:56:59.9630000Z',
                'status': 'sent',
                'subject': 'Simple Signing Example',
                'template_id': None,
                'template_uri': None,
                '_configuration': None
                }
    _configuration = None
    completed_date_time = None
    created_date_time = '2023-02-02T09:56:58.7600000Z'
    envelope_id = 'bef86597-7e1d-4a77-a778-e8f2fe004cf3'
    envelope_uri = '/envelopes/bef86597-7e1d-4a77-a778-e8f2fe004cf3'
    expire_date_time = '2023-06-02T14:02:05.4753500Z'
    folder_id = None
    folder_uri = None
    is21_cfr_part11 = 'false'
    last_modified_date_time = '2023-02-02T09:56:59.0000000Z'
    owner_name = 'Muhammad Shoaib'
    recipients = None
    recipients_uri = '/envelopes/bef86597-7e1d-4a77-a778-e8f2fe004cf3/recipients'
    sender_company = None
    sender_email = 'muhammad.shoaib@crowdbotics.com'
    sender_name = 'Muhammad Shoaib'
    sender_user_id = 'c941de4f-d46f-4f36-b0b5-0ca24c2003fa'
    sent_date_time = '2023-02-02T09:56:59.9630000Z'
    status = 'sent'
    subject = 'Simple Signing Example'
    template_id = None
    template_uri = None


class RetrieveAllEnvelope:
    folder_items = [FolderItem()]


class AuthTokenTestCase(APITestCase):

    @mock.patch('modules.django_docusign.docusign.viewsets.ApiClient.request_jwt_user_token')
    def test_get_auth_token(self, request_jwt_user_token_mock):
        responses = {
            'access_token': 'eyJ0eXAiOiJNVCIsImFsZyI6IlJTMjU2Iiwia2lkIjoiNjgxODVmZjEtNGU1MS00Y2U5LWFmMWMtNjg5ODEyMjAzMzE3In0.AQoAAAABAAUABwAAJ3zF7wTbSAgAAI9AJ_gE20gCAE_eQclv1DZPsLUMokwgA_oVAAEAAAAYAAIAAAAFAAAAHQAAAA0AJAAAAGJlMTA2ZTJmLWYxYTQtNGMyYy1hZTVmLTAxY2FjNGU0Mjk2YSIAJAAAAGJlMTA2ZTJmLWYxYTQtNGMyYy1hZTVmLTAxY2FjNGU0Mjk2YRIAAQAAAAYAAABqd3RfYnIjACQAAABiZTEwNmUyZi1mMWE0LTRjMmMtYWU1Zi0wMWNhYzRlNDI5NmE.D6iN1O24Zz--0sRWBDj7Gm9MXj_m0APeZ6cmBzxPmfOCr22S5hjc7LN6ppOBXknfAd4e2MqbaZNEJo3UXW-ZFIyE1SPDVfe8PhUSBGLdw3kJL79K0z_9E7SPAZ7ve9YLeV5quWcdpUH_U6Qs_sraD3R4oZTxQL6UcXvpCenFEmnp196GD--wtkCLVGr0x-qPtE6qTW4uTEqBUfviGkmLS-audLE3Gv8U1KhfNPjU5ImF8ZgR6NhLLTuV5g6nA_QZoJv1mR2kFpAIb5L3CJ_Uqsx4AOC18EVO0RmdEQ8DqPpynCoJNyVU8p61evmD70iw6hvB_R55WzF1W_Wwxxwzlg',
            'expires_in': '3600', 'scope': 'signature impersonation', 'token_type': 'Bearer'}
        dict_response = AttrDict(responses)
        request_jwt_user_token_mock.return_value = dict_response
        url = "/modules/docusign/auth/token/"
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        request_jwt_user_token_mock.assert_called_once()

    def test_get_auth_token_without_client_id(self):
        url = "/modules/docusign/auth/token/"
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)


class CreateEnvelopeTestCase(APITestCase):
    def setUp(self):
        self.client.credentials(
            HTTP_AUTHORIZATION='Bearer ' + "eyJ0eXAiOiJNVCIsImFsZyI6IlJTMjU2Iiwia2lkIjoiNjgxODVmZjEtNGU1MS00Y2U5LWFmMWMtNjg5ODEyMjAzMzE3In0.AQoAAAABAAUABwAAJ3zF7wTbSAgAAI9AJ_gE20gCAE_eQclv1DZPsLUMokwgA_oVAAEAAAAYAAIAAAAFAAAAHQAAAA0AJAAAAGJlMTA2ZTJmLWYxYTQtNGMyYy1hZTVmLTAxY2FjNGU0Mjk2YSIAJAAAAGJlMTA2ZTJmLWYxYTQtNGMyYy1hZTVmLTAxY2FjNGU0Mjk2YRIAAQAAAAYAAABqd3RfYnIjACQAAABiZTEwNmUyZi1mMWE0LTRjMmMtYWU1Zi0wMWNhYzRlNDI5NmE.D6iN1O24Zz--0sRWBDj7Gm9MXj_m0APeZ6cmBzxPmfOCr22S5hjc7LN6ppOBXknfAd4e2MqbaZNEJo3UXW-ZFIyE1SPDVfe8PhUSBGLdw3kJL79K0z_9E7SPAZ7ve9YLeV5quWcdpUH_U6Qs_sraD3R4oZTxQL6UcXvpCenFEmnp196GD--wtkCLVGr0x-qPtE6qTW4uTEqBUfviGkmLS-audLE3Gv8U1KhfNPjU5ImF8ZgR6NhLLTuV5g6nA_QZoJv1mR2kFpAIb5L3CJ_Uqsx4AOC18EVO0RmdEQ8DqPpynCoJNyVU8p61evmD70iw6hvB_R55WzF1W_Wwxxwzlg")

    @mock.patch('modules.django_docusign.docusign.viewsets.EnvelopesApi.create_envelope')
    def test_CreateEnvelope(self, create_envelope_mock):
        responses = {'envelope_id': 'bef86597-7e1d-4a77-a778-e8f2fe004cf3',
                     'uri': '/envelopes/bef86597-7e1d-4a77-a778-e8f2fe004cf3',
                     'status_date_time': '2023-02-02T09:56:59.9630000Z', 'status': 'sent'}
        dict_response = AttrDict(responses)
        create_envelope_mock.return_value = dict_response
        url = "/modules/docusign/envelope/create/"
        data = {
            "documents": [
                {
                    "documentBase64": "",
                    "documentId": 3,
                    "fileExtension": "pdf",
                    "name": "document"
                }
            ],
            "emailSubject": "Simple Signing Example",
            "recipients": {
                "signers": [
                    {
                        "email": "yebica2686@areosur.com",
                        "name": "yebica",
                        "recipientId": "1"
                    }
                ]
            },
            "status": "sent"
        }
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['envelope_id'], responses['envelope_id'])
        create_envelope_mock.assert_called_once()

    def test_CreateEnvelope_without_access_token(self):
        self.client.force_authenticate(token=None)
        url = "/modules/docusign/envelope/create/"
        data = {}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_CreateEnvelope_with_expired_token(self):
        url = "/modules/docusign/envelope/create/"
        data = {
            "documents": [
                {
                    "documentBase64": "",
                    "documentId": 3,
                    "fileExtension": "pdf",
                    "name": "document"
                }
            ],
            "emailSubject": "Simple Signing Example",
            "recipients": {
                "signers": [
                    {
                        "email": "yebica2686@areosur.com",
                        "name": "yebica",
                        "recipientId": "1"
                    }
                ]
            },
            "status": "sent"
        }
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)


class RetrieveEnvelopeTestCase(APITestCase):
    def setUp(self):
        self.client.credentials(
            HTTP_AUTHORIZATION='Bearer ' + "eyJ0eXAiOiJNVCIsImFsZyI6IlJTMjU2Iiwia2lkIjoiNjgxODVmZjEtNGU1MS00Y2U5LWFmMW"
                                           "MtNjg5ODEyMjAzMzE3In0.AQoAAAABAAUABwAAJ3zF7wTbSAgAAI9AJ_gE20gCAE_eQclv1DZ"
                                           "PsLUMokwgA_oVAAEAAAAYAAIAAAAFAAAAHQAAAA0AJAAAAGJlMTA2ZTJmLWYxYTQtNGMyYy1h"
                                           "ZTVmLTAxY2FjNGU0Mjk2YSIAJAAAAGJlMTA2ZTJmLWYxYTQtNGMyYy1hZTVmLTAxY2FjNGU0M"
                                           "jk2YRIAAQAAAAYAAABqd3RfYnIjACQAAABiZTEwNmUyZi1mMWE0LTRjMmMtYWU1Zi0wMWNhYz"
                                           "RlNDI5NmE.D6iN1O24Zz--0sRWBDj7Gm9MXj_m0APeZ6cmBzxPmfOCr22S5hjc7LN6ppOBXknf"
                                           "Ad4e2MqbaZNEJo3UXW-ZFIyE1SPDVfe8PhUSBGLdw3kJL79K0z_9E7SPAZ7ve9YLeV5quWcdpU"
                                           "H_U6Qs_sraD3R4oZTxQL6UcXvpCenFEmnp196GD--wtkCLVGr0x-qPtE6qTW4uTEqBUfviGkmLS"
                                           "-audLE3Gv8U1KhfNPjU5ImF8ZgR6NhLLTuV5g6nA_QZoJv1mR2kFpAIb5L3CJ_Uqsx4AOC18EV"
                                           "O0RmdEQ8DqPpynCoJNyVU8p61evmD70iw6hvB_R55WzF1W_Wwxxwzlg")

    @mock.patch('modules.django_docusign.docusign.viewsets.EnvelopesApi.get_envelope')
    def test_retrieve_envelope_details(self, get_envelope_mock):
        responses = {'envelope_id': '6babd0ef-626c-45f8-85b7-6781788f319e',
                     'attachments_uri': '/envelopes/6babd0ef-626c-45f8-85b7-6781788f319e/attachments',
                     'created_date_time': '2022-12-20T12:38:31.2300000Z', 'delivered_date_time': None,
                     'declined_date_time': None, 'deleted_date_time': None, 'completed_date_time': None,
                     'initial_sent_date_time': '2022-12-20T12:38:32.0570000Z',
                     'last_modified_date_time': '2022-12-20T12:38:31.2300000Z',
                     'notification_uri': '/envelopes/6babd0ef-626c-45f8-85b7-6781788f319e/notification',
                     'recipients_uri': '/envelopes/6babd0ef-626c-45f8-85b7-6781788f319e/recipients',
                     'sender': {'account_id': 'e6225d9c-96e5-4cdf-9f71-a81087f72c3b',
                                'email': 'muhammad.shoaib@crowdbotics.com', 'user_name': 'Muhammad Shoaib'},
                     'sent_date_time': '2022-12-20T12:38:32.0570000Z', 'status': 'sent',
                     'status_changed_date_time': '2022-12-20T12:38:32.0570000Z',
                     'templates_uri': '/envelopes/6babd0ef-626c-45f8-85b7-6781788f319e/templates'}
        dict_response = AttrDict(responses)
        get_envelope_mock.return_value = dict_response
        url = "/modules/docusign/envelope/retrieve/"
        params = {"envelope_id": "6babd0ef-626c-45f8-85b7-6781788f319e"}
        response = self.client.get(url, params)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(responses['envelope_id'], response.data['envelope_id'])
        get_envelope_mock.assert_called_once()

    def test_retrieve_envelope_details_without_access_token(self):
        self.client.force_authenticate(token=None)
        url = "/modules/docusign/envelope/retrieve/"
        params = {}
        response = self.client.get(url, params, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_retrieve_envelope_details_with_expired_token(self):
        url = "/modules/docusign/envelope/retrieve/"
        params = {"envelope_id": "6babd0ef-626c-45f8-85b7-6781788f319e"}
        response = self.client.get(url, params, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)


class DownloadEnvelopeTestCase(APITestCase):
    def setUp(self):
        self.client.credentials(
            HTTP_AUTHORIZATION='Bearer ' + "eyJ0eXAiOiJNVCIsImFsZyI6IlJTMjU2Iiwia2lkIjoiNjgxODVmZjEtNGU1"
                                           "MS00Y2U5LWFmMWMtNjg5ODEyMjAzMzE3In0.AQoAAAABAAUABwAAJ3zF7wTbSAgAAI9AJ_g"
                                           "E20gCAE_eQclv1DZPsLUMokwgA_oVAAEAAAAYAAIAAAAFAAAAHQAAAA0AJAAAAGJlMTA2ZTJ"
                                           "mLWYxYTQtNGMyYy1hZTVmLTAxY2FjNGU0Mjk2YSIAJAAAAGJlMTA2ZTJmLWYxYTQtNGMyYy1h"
                                           "ZTVmLTAxY2FjNGU0Mjk2YRIAAQAAAAYAAABqd3RfYnIjACQAAABiZTEwNmUyZi1mMWE0LTRjMmMt"
                                           "YWU1Zi0wMWNhYzRlNDI5NmE.D6iN1O24Zz--0sRWBDj7Gm9MXj_m0APeZ6cmBzxPmfOCr22S5h"
                                           "jc7LN6ppOBXknfAd4e2MqbaZNEJo3UXW-ZFIyE1SPDVfe8PhUSBGLdw3kJL79K0z_9E7SPAZ7ve9"
                                           "YLeV5quWcdpUH_U6Qs_sraD3R4oZTxQL6UcXvpCenFEmnp196GD--wtkCLVGr0x-qPtE6qTW4uTE"
                                           "qBUfviGkmLS-audLE3Gv8U1KhfNPjU5ImF8ZgR6NhLLTuV5g6nA_QZoJv1mR2kFpAIb5L3CJ_Uqsx"
                                           "4AOC18EVO0RmdEQ8DqPpynCoJNyVU8p61evmD70iw6hvB_R55WzF1W_Wwxxwzlg")

    @mock.patch('modules.django_docusign.docusign.viewsets.EnvelopesApi.get_document')
    def test_get_document(self, get_document_mock):
        responses = '/tmp/02022023_132520_570922_document.pdf'
        get_document_mock.return_value = responses
        url = "/modules/docusign/envelope/download/"
        params = {
            "envelope_id": "68d29314-8a11-43a2-951f-521164d7071f",
            "document_id": "1"
        }
        response = self.client.get(url, params)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        get_document_mock.assert_called_once()

    def test_get_document_without_access_token(self):
        self.client.force_authenticate(token=None)
        url = "/modules/docusign/envelope/download/"
        params = {
            "envelope_id": "68d29314-8a11-43a2-951f-521164d7071f",
            "document_id": "1"
        }
        response = self.client.get(url, params)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_get_document_with_expired_token(self):
        url = "/modules/docusign/envelope/download/"
        params = {
            "envelope_id": "68d29314-8a11-43a2-951f-521164d7071f",
            "document_id": "1"
        }
        response = self.client.get(url, params)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)


class RetrieveAllEnvelopeTestCase(APITestCase):
    def setUp(self):
        self.client.credentials(
            HTTP_AUTHORIZATION='Bearer ' + "eyJ0eXAiOiJNVCIsImFsZyI6IlJTMjU2Iiwia2lkIjoiNjgxODVmZjEtNGU1MS00Y2U5LWFmMWMtNjg5ODEyMjAzMzE3In0.AQoAAAABAAUABwAAJ3zF7wTbSAgAAI9AJ_gE20gCAE_eQclv1DZPsLUMokwgA_oVAAEAAAAYAAIAAAAFAAAAHQAAAA0AJAAAAGJlMTA2ZTJmLWYxYTQtNGMyYy1hZTVmLTAxY2FjNGU0Mjk2YSIAJAAAAGJlMTA2ZTJmLWYxYTQtNGMyYy1hZTVmLTAxY2FjNGU0Mjk2YRIAAQAAAAYAAABqd3RfYnIjACQAAABiZTEwNmUyZi1mMWE0LTRjMmMtYWU1Zi0wMWNhYzRlNDI5NmE.D6iN1O24Zz--0sRWBDj7Gm9MXj_m0APeZ6cmBzxPmfOCr22S5hjc7LN6ppOBXknfAd4e2MqbaZNEJo3UXW-ZFIyE1SPDVfe8PhUSBGLdw3kJL79K0z_9E7SPAZ7ve9YLeV5quWcdpUH_U6Qs_sraD3R4oZTxQL6UcXvpCenFEmnp196GD--wtkCLVGr0x-qPtE6qTW4uTEqBUfviGkmLS-audLE3Gv8U1KhfNPjU5ImF8ZgR6NhLLTuV5g6nA_QZoJv1mR2kFpAIb5L3CJ_Uqsx4AOC18EVO0RmdEQ8DqPpynCoJNyVU8p61evmD70iw6hvB_R55WzF1W_Wwxxwzlg")

    @mock.patch('modules.django_docusign.docusign.viewsets.FoldersApi.search')
    def test_get_retrieve_all_evelope(self, search_mock):
        search_mock.return_value = RetrieveAllEnvelope()
        url = "/modules/docusign/envelope/retrieve-all/"
        params = {
            "folder_value": "out_for_signature"
        }
        response = self.client.get(url, params)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        search_mock.assert_called_once()

    def test_get_retrieve_all_envelope_without_token(self):
        self.client.force_authenticate(token=None)
        url = "/modules/docusign/envelope/retrieve-all/"
        params = {
            "folder_value": "out_for_signature"
        }
        response = self.client.get(url, params)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_get_retrieve_all_envelope_with_expired_token(self):
        url = "/modules/docusign/envelope/retrieve-all/"
        params = {
            "folder_value": "out_for_signature"
        }
        response = self.client.get(url, params)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)